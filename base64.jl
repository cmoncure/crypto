...
function base64_encode(bytes_input::Array{Uint8})
  ...
  padding::Int = input_len % 3 == 0 ? 0 : abs((int(input_len) % 3) - 3)
...
#  bytes_input_len -= padding
  ...
  bytes_output = Array(Uint8, bytes_output_len)
  println("input length: ", bytes_input_len)
  println("output length: ", bytes_output_len)
  i = 0
  while o_idx <= bytes_output_len
    buf = 0
    i = 3
    while i > -1 && i_idx <= bytes_input_len
#      println(i_idx, bytes_input[i_idx])
...
#      println("output byte ", o_idx, ": ", bits(bytes_output[o_idx]))
      o_idx += 1
      i -= 1
    end
  end
...
teststr = hex2bytes("14fb9c03d97e")
a = base64_encode(teststr)
ascii(a)
base64_decode(a)
